...
| `claude -p "/second-opinion …"` fan-out | `agent -p --trust -f "/second-opinion …"` |
...
Model is **`opus`** for every agent. Effort is inherited from the session — the
`Agent` tool has no effort parameter, so if the user asked for "high," confirm
their session is set to it rather than claiming you applied it.

## Step 1: Build the canvas

```bash
canvas=$(mktemp -d -t debate-XXXXXX)
```
...
Launch all N agents in **one response block**, all `run_in_background: true`,
...
Once every agent has completed, launch N fresh agents — one per position, each
told which position it now owns.
...
- **Do not poll.** You are notified as each agent finishes. Never read an
  agent's `.output` file — that is the raw transcript and it will overflow you.
