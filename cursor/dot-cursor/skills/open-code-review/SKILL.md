...
compatibility: >
  ...
  @alibaba-group/open-code-review` or GitHub release binary). The configured
  model is the local `cursor-agent` provider: `ocr-cursor-agent` on
  127.0.0.1:39221, which shells out to `agent -p --mode ask`.
metadata:
  author: alibaba
...
OCR's model is already `cursor-agent` (`~/.opencodereview/config.json`: OpenAI protocol at `http://127.0.0.1:39221/v1`). That process is `/home/mdelrio/dotfiles/bin/ocr-cursor-agent`. Each completion runs `/home/mdelrio/.local/bin/agent -p --mode ask --trust --output-format text` and the shim returns native `tool_calls`. Do not use `ocr delegate`. Do not pass `--provider` unless the user names a different model.

Ensure the shim is up, then run the OCR command. **Always pass business context via `--background`** when available:

```bash
/home/mdelrio/dotfiles/bin/ocr-cursor-agent health || /home/mdelrio/dotfiles/bin/ocr-cursor-agent start
```
...
- **Cursor agent provider must be listening** — `ocr review` fails if `http://127.0.0.1:39221/v1` is down. Run `ocr-cursor-agent start` (Troubleshooting).
...
Start the local Cursor agent provider and retest. Do not point OCR at another vendor unless the user asks.

```bash
/home/mdelrio/dotfiles/bin/ocr-cursor-agent start
ocr llm test
```
