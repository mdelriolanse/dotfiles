...
Parse arguments from the user's message (whitespace-split):
...
Dispatch `question` once with two options:
...
not to fire (no `question` since `behind == 0`), but the
...
if $fetch_ok; then
    ...
    if behind=$(git rev-list --count "HEAD..$base_branch" 2>/dev/null); then
        ...
        if [[ "$behind" == "0" ]]; then
            ...
            # resolved to 0. The `question` below won't fire (gated
...
overflow) so the user sees them before the `question` fires:
...
Then dispatch `question` with **four single-select options**,
...
2. **Dispatch `question`** with options:
   ...
   - "✎ Edit the hint" — captures a free-form replacement string via
     a follow-up `question`, then promotes via
