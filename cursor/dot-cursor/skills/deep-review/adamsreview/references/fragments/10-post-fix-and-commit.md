...
plain overlap, offer a three-way `question` before abort.
...
Dispatch `question` with three options; Abort highlighted as default:
...
Dispatch ONE `Task` tool-use with `subagent_type: general`,
.
