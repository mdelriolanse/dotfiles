#!/usr/bin/env bash
# claude-md-paths.sh — walk-up CLAUDE.md / AGENTS.md finder (DESIGN §21.7, §23).
#
# For each input file, walk from that file's parent directory up to
# --repo-root, collecting directories that contain a CLAUDE.md or AGENTS.md.
# The repo-root CLAUDE.md/AGENTS.md (if present) is always included. Output
# is one absolute path per line, deduped, sorted by path depth (root-first).
#
# AGENTS.md (opencode convention) takes priority over CLAUDE.md at any
# given directory level — only one is emitted per directory.
#
...
# have no CLAUDE.md/AGENTS.md anywhere); 1 --repo-root missing, not a directory,
...
# AGENTS.md (priority) or CLAUDE.md found along the way. Dedup and
# root-first sort happen at the end via `sort -u` — avoids the macOS
# Bash 3.2 associative-array limitation.
{
    for f in "${FILES[@]}"; do
        if [[ "$f" = /* ]]; then
            abs="$f"
        ...
        cur="$parent"
        while : ; do
            # AGENTS.md (opencode convention) takes priority over CLAUDE.md
            if [[ -f "$cur/AGENTS.md" ]]; then
                echo "$cur/AGENTS.md"
            elif [[ -f "$cur/CLAUDE.md" ]]; then
                echo "$cur/CLAUDE.md"
            fi
    ...
    # Always include repo-root AGENTS.md (priority) or CLAUDE.md if present
    # (covers the case where no touched file lives under a subtree).
