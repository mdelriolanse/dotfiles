...
|---|---|
| `Agent` tool | a fresh `opencode run --auto` process |
| `run_in_background: true` | shell-background `opencode run ... &`, then `wait $!` |
| `--add-dir` / workspace | `--dir <container>` |
...
`opencode run --auto` sub-sessions for isolated work. All sub-sessions are
...
- RED/GREEN agents run as `opencode run --auto -m anthropic/claude-opus-4-8`
...
- **`opencode run --auto` sub-sessions don't enter plan mode.** `-p` (print
  ...
  `--auto` to every `opencode run` call (belt and suspenders with the
  global config).
