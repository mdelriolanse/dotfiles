...
`question` dispatches; backgrounded shells leave the orchestrator
...
Parse arguments from the user's message for `--full`. Set
`force_full=true/false` in your context.

Any token not recognized as `--full` is unexpected — stop and ask the
user to clarify.
...
and behind-count; orchestrator owns `question`.
...
do NOT dump the diff). Then use `question` once with three options:
...
Dispatch with the `Task` tool. After the sub-agent returns,
...
If a comment id is returned, run `question` with three choices:
