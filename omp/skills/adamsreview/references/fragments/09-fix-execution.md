...
Fan out one `Task` tool-use per group, all in a single orchestrator
turn. Each agent uses `subagent_type: general`,
...
Then fire all `group_count` Task tool-use blocks in the same turn.
