...
Model is **`glm-5-2-nvfp4:high`** (GLM 5.2 at high thinking) for every agent.
Effort is set via the `:high` thinking suffix; pass `:max` for higher.
...
Once every agent has completed, launch N fresh `omp -p` processes — one per
