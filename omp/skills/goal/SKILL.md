...
Path: make the change → run the affected test or command end-to-end to prove it
works → report. If there is nothing to run (docs, config with no runtime
surface), say so plainly instead of claiming it passed.
...
A one-shot that fails its verification check has told you it was mis-triaged. Do
