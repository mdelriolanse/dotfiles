...
`omp -p --no-session` sub-sessions for isolated work. All sub-sessions are
...
Each sub-session is a non-interactive `omp -p` process (no popup):
...
parallel with Session 4. Same pattern: `omp -p --no-session --cwd "$CONTAINER"`
...
- **`omp -p --no-session` sub-sessions don't enter plan mode.** `-p` (print
