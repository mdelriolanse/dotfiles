...
scrape — there is no omp path through it. Track B is the native way to get
...
| **B — code-reviewer** | background `omp -p` | `--extra` |
| **C — PRD conformance** | background `omp -p` | `--prd` given |
...
comes from shell-background `omp -p` processes launched *before* starting A.
...
```bash
for i in $(seq 1 "$K"); do
  DEEP_REVIEW_INNER=1 omp -p --no-session --model glm-5-2-nvfp4:high \
...
omp has no `run_in_background` on `Task`. Launch B (and C when `--prd` is
set) as **shell-background `omp -p` jobs** in one block, then start track A
while they run.
...
```bash
omp -p --no-session --model glm-5-2-nvfp4:high --cwd "$PWD" \
