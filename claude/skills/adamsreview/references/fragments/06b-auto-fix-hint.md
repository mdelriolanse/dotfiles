...
For each chunk file, launch ONE `Task` tool-use with ,
`subagent_type: general`. Each gen sub-agent receives the
...
in §5.5.2 are skipped here), launch ONE `Task` tool-use with
. Each verify sub-agent receives:
