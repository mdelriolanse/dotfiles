...
Six internal lenses run in parallel to produce candidate findings. Each
...
send every applicable lens's `Task` tool-use block inside a single
orchestrator turn. The tool dispatcher then runs them concurrently. Collecting the
...
**MANDATORY — Read every lens file before dispatch.** Before the dispatch
turn below, you MUST have issued `Read` tool-uses for
`references/fragments/lens-prompts/_shared-invariants.md` and every
applicable `references/fragments/lens-prompts/L<N>.md` file (L1.md through
L6.md). Do NOT proceed based on memory, summary, or what you think a lens
does. The content of these files IS the prompt that goes to the sub-agent.
Skipping them causes hallucinated prompts, wrong `source_family` values, and
schema-rejected candidates.

Skipped lenses get a one-line note in `trace.md`:
```
Phase 1: L2/L5/L6 skipped (trivial_mode=true)
```
L7 (holistic) is not available in the opencode port — ensemble support has
not been ported. Document it with `log-phase.sh --summary`.
L7 is always skipped with a note: `Phase 1: L7 skipped (not available in opencode port)`.
...
diff; L2 additionally reads surrounding files and uses git blame / git log.
...
L3/L4/L5/L6 consume `claude_md_paths`.

Shared invariants: Read `references/fragments/lens-prompts/_shared-invariants.md`
...
same way `references/fragments/01-codex-detection.md` §1.2c handles it.)
...
The lens-body files (`references/fragments/lens-prompts/L<N>.md`) are read in
...
- "ONLY the diff" vs. "diff plus surrounding files / git blame / git
  log" — lens-specific reading scope (L1 is diff-only; L2 walks
  outward).
- CLAUDE.md reading — L3, L4, L5 consume `$claude_md_paths`; others
  don't.
- "Over-flag; Phase 3 will filter" directive — appears in L1, L2, L6
  where over-flag is the intended posture; L3/L4/L5 don't carry it.
...
### 1.3. Dispatch the lenses (one turn, one Task call per applicable lens)

**Parallel dispatch — load-bearing.** Read all applicable
`references/fragments/lens-prompts/L<N>.md` files plus
`references/fragments/lens-prompts/_shared-invariants.md` first (these `Read`
tool-uses can run in parallel within one orchestrator turn). Then
issue EVERY applicable lens's `Task` tool-use in a SINGLE
orchestrator turn so they run concurrently. The per-lens sub-sections
...
six serial action targets. The unambiguous action target is the
"#### Dispatch turn" sub-section after L6. Treating each per-lens
...
> applicable lens's `Task` block in the single `#### Dispatch turn` at the
> end of this section — one orchestrator turn, not seven. Phase 1 latency is
> `max(lens_durations)`, not `sum(lens_durations)`.

Dispatch spec: `subagent_type: general`.

Prompt body: `references/fragments/lens-prompts/L1.md` (read in step 1.3's bulk
...
Dispatch spec: `subagent_type: general`. The
...
Prompt body: `references/fragments/lens-prompts/L2.md` (read in step 1.3's bulk
...
Prompt body: `references/fragments/lens-prompts/L3.md` (read in step 1.3's bulk
...
Prompt body: `references/fragments/lens-prompts/L4.md` (read in step 1.3's bulk
...
Prompt body: `references/fragments/lens-prompts/L5.md` (read in step 1.3's bulk
pre-read; its content is the L5 prompt body verbatim — the canonical
content; `references/fragments/lens-ux-reference.md` is a redundant duplicate kept
...
Prompt body: `references/fragments/lens-prompts/L6.md` (read in step 1.3's bulk
pre-read; its content is the L6 prompt body verbatim — the canonical
content; `references/fragments/lens-security-reference.md` is a redundant
...
These are working-context value initializations per the operational
rules ("Working set lives in-prompt, not shell vars"),
not `Bash` tool-uses; the orchestrator records them in-context
*before* issuing the `Task` blocks below.

**Dispatch.** With every applicable lens's spec assembled (L1–L6
sub-sections above), issue every applicable lens's `Task` tool-use
...
1. **Log tokens first** (§24.4 — "cost accounted even for failed agents").
   Parse the sub-agent's `the token count from the Task result` block. If the
   Task tool result exposes a structured `usage` field directly, prefer
