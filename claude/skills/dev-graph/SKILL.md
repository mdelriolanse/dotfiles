...
`claude -p` sub-sessions for isolated work. All sub-sessions are
...
- RED/GREEN agents run as `claude -p --model opus`
...
Each sub-session is a non-interactive `claude -p` process (no popup):
...
parallel with Session 4. Same pattern: `claude -p --add-dir "$CONTAINER"`
...
- **`claude -p` sub-sessions don't enter plan mode.** `-p` (print
  ...
  `--dangerously-skip-permissions` to every `claude -p` call (belt and suspenders with the
  global config).
