...
Your job is to notice when a completed task represents a recurring workflow that would benefit from being saved as a reusable Claude Code skill. If so, suggest creating a `SKILL.md` — but only at the right time and in the right way.
...
|---|---|
| Specific to a repo or tech stack (e.g., Django migrate + seed, this project's CI rules) | `project` (`.claude/skills/...`) |
| Generic and reusable across projects (e.g., GitHub PR review, uv setup, Docker build pattern) | `global` (`~/.claude/skills/...`) |
...
After writing it, remind the user to quit and restart Claude Code so the new skill is loaded.
