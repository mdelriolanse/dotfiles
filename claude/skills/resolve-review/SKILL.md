...
Invoke the `bg-subagent` skill (one background agent) to fix **all** trivial
findings in one pass. **Pull the task prompt directly from the report file** —
quote each trivial finding's own text verbatim (its file:line and rationale) as
the agent's brief, rather than paraphrasing. The report is self-contained; the
cold agent should read the same words the report gives.
...
lens**: invoke the `ponytail:ponytail` skill and, for each HITL finding, state
the laziest correct fix — the fewest-lines approach that holds, naming any
ceiling. Present it as a plain `Recommended:` line under the finding.
...
When the background agent finishes, the harness re-invokes you — relay what it
changed per finding; the user never sees its output directly.
