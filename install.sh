...
# runtime state, so we must never replace the whole directory. Skills, CLAUDE.md,
# MCP servers, and the ponytail plugin are derived from the opencode source in the
# dedicated Claude Code section below.
...
# ---------------------------------------------------------------------------
# 6. Claude Code — reuse the opencode source (skills, rules, MCP, ponytail).
#    ~/.claude/ is a real dir with runtime state, so we only add/derive into it,
#    never symlink the whole dir. Every step is idempotent and content-agnostic
#    (reads whatever instructions / MCP servers / skills exist at install time),
#    and no-ops cleanly when the `claude` CLI is absent.
# ---------------------------------------------------------------------------
if command -v claude >/dev/null 2>&1; then
  info "Configuring Claude Code ..."

  # 6a. Skills: symlink every opencode skill into ~/.claude/skills.
  if [ -x "$REPO_DIR/opencode/scripts/link-skills.sh" ]; then
    "$REPO_DIR/opencode/scripts/link-skills.sh" >/dev/null 2>&1 \
      && ok "claude skills linked into ~/.claude/skills" \
      || warn "claude skill linking failed (non-fatal)"
  fi

  # 6b. Rules: generate ~/.claude/CLAUDE.md as a manifest of @-imports pointing at
  #     the opencode instructions. Content stays in opencode (true DRY); @-imports
  #     resolve live at runtime via the ~/.config/opencode symlink from step 1.
  CLAUDE_MD="$HOME/.claude/CLAUDE.md"
  GEN_MARK="Generated by dotfiles/install.sh"
  if [ -d "$REPO_DIR/opencode/instructions" ]; then
    mkdir -p "$HOME/.claude"
    # Back up any hand-written CLAUDE.md before overwriting with the generated one.
    if [ -e "$CLAUDE_MD" ] && ! head -n1 "$CLAUDE_MD" | grep -qF "$GEN_MARK"; then
      mkdir -p "$BACKUP_DIR$HOME/.claude"
      mv "$CLAUDE_MD" "$BACKUP_DIR$CLAUDE_MD"
      warn "backed up existing ~/.claude/CLAUDE.md -> ${BACKUP_DIR/#$HOME/\~}$CLAUDE_MD"
    fi
    {
      echo "<!-- $GEN_MARK — do not edit. Source rules live in ~/.config/opencode/. -->"
      echo "<!-- Re-run ./install.sh to regenerate after adding/removing instructions. -->"
      echo
      [ -f "$REPO_DIR/opencode/AGENTS.md" ] && echo "@~/.config/opencode/AGENTS.md"
      for f in "$REPO_DIR"/opencode/instructions/*.md; do
        [ -e "$f" ] || continue
        echo "@~/.config/opencode/instructions/$(basename "$f")"
      done
    } > "$CLAUDE_MD"
    ok "generated ~/.claude/CLAUDE.md (@-imports from ~/.config/opencode)"
  fi

  # 6c. MCP: register the same servers materialized for Cursor in step 4 (real
  #     secret values already substituted) into Claude's user scope. remove-then-
  #     add keeps it idempotent; stdio -> add-json, remote {url} -> http transport.
  if command -v jq >/dev/null 2>&1 && [ -f "$MCP_REAL" ]; then
    while IFS= read -r name; do
      [ -n "$name" ] || continue
      srv="$(jq -c --arg n "$name" '.mcpServers[$n]' "$MCP_REAL")"
    ...
    done < <(jq -r '.mcpServers | keys[]' "$MCP_REAL" 2>/dev/null)
  else
    warn "claude MCP skipped (need jq + cursor/dot-cursor/mcp.json — see step 4)"
  fi

  # 6d. ponytail: install its native Claude Code plugin (commands, hooks, modes).
  PONY_MP="$REPO_DIR/opencode/ponytail/.claude-plugin"
  if [ -d "$PONY_MP" ]; then
    claude plugin marketplace add "$PONY_MP" >/dev/null 2>&1 || true
    if claude plugin list 2>/dev/null | grep -qi "ponytail"; then
      ok "ponytail Claude Code plugin present"
    else
      claude plugin install ponytail@ponytail -s user >/dev/null 2>&1 \
        && ok "installed ponytail Claude Code plugin" \
        || warn "ponytail plugin install failed (non-fatal)"
    fi
  else
    warn "ponytail plugin skipped — $PONY_MP missing (init the git submodule?)"
  fi
else
...
Next steps:
  ...
  6) Claude Code: verify with 'claude mcp list' and 'claude plugin list'
     (~/.claude/CLAUDE.md + skills are regenerated on each ./install.sh run).
